$.AwarePager = function () {
    ...
    function calculatePagerLink(pageNo, currentPage) {
        var url = window.location.href;
        var pageFilter = $("#PageFilters").val() || "";
        var queryString = pageFilter + "&" + (url.indexOf("?") > -1 ? url.split("?")[1] : "");

        var queryStrings = queryString.split("&"); queryString = "";
        var queryTypes = [];

        for (var i = 0; i < queryStrings.length; i++) {
            var value = queryStrings[i];
            var type = value.split("=")[0];
            if (value.length > 0 && $.inArray(type, queryTypes) == -1 && type != pagerDefinition.urlName) {
                queryString += value + "&";
                queryTypes.push(type);
            }
        }
        return url.split("?")[0] + "?" + queryString + pagerDefinition.urlName + "=" + pageNo;
    };
    ...
    function drawPager() {
        ...
        if (currentPage > pagerDefinition.linkCount) {
            ...
            var beforeLink = calculatePagerLink(pageIndex, currentPage);
            pagerHtml += '<li class="paginate_button page"><a href="' + beforeLink + '">&lt;</a></li>';
        }
        ...
        for (i = 0; i < pagerDefinition.linkCount; i++) {
            ...
            if (pageIndex <= pagerCount) {
                var pageLink = calculatePagerLink(pageIndex, currentPage);
        ...
        if (Math.ceil(currentPage / pagerDefinition.linkCount) < Math.ceil(pagerCount / pagerDefinition.linkCount) && pagerCount > pagerDefinition.linkCount) {
            ...
            if (pageIndex <= pagerCount) {
                var afterLink = calculatePagerLink(pageIndex + 1, currentPage);
                pagerHtml += '<li class="paginate_button page"><a href="' + afterLink + '">&gt;</a></li>';
            }
        }
        pagerHtml += '</ul>';
...
$.Aware = function () {
    ...
    this.validate = function (area, pos) {
        var validated = true;
        $(area).find(".error").removeClass("error");
        $(area).find(".validate").each(function () {
            var hasError = false;
            var message = "";
            var field = $(this);
            ...
            if ($(field) == undefined || (!$(field).hasClass('sbx-check') && !$(field).hasClass('cbx-check') && !$(field).hasClass('validate-file') && value.length == 0)) { //null-check
                hasError = true;
            }
            else if ($(field).hasClass("email-check") && !current[0].isValidEmail(value)) { //email check
                hasError = true;
                message = "Belirttiğiniz e-posta adresi geçerli değil!";
            }
            else if ($(field).hasClass("sbx-check")) { //selectbox check
                if ($(field).hasClass("selecto-wrap")) {
                    var hiddenValue = $(field).find("input[type='hidden']").val();
                    hasError = hiddenValue.length == 0 || hiddenValue == "0";
                }
                else if (($(field).data("id") == 0 || $(field).val() == 0)) {
                    hasError = true;
                }
                if (hasError) { message = "Seçim yapmadınız!"; }
            } else if ($(field).hasClass("must-equal") || $(field).hasClass("not-equal")) { //check equality or unequality
                var fieldToCompare = $(area).find("#" + $(field).data("compare-id"));
                if ($(fieldToCompare) == undefined || ($(field).hasClass("must-equal") && $(fieldToCompare).val() != value) || ($(field).hasClass("not-equal") && $(fieldToCompare).val() == value)) {
                    message = $(field).data("comp-msg");
                    hasError = true;
                }
            } else if ($(field).hasClass("cbx-check")) { //checkbox
                if ($(field) == undefined || !$(field).is(':checked')) {
                    hasError = true;
                }
            }
            else if (allowFileCheck && $(field).find("input[type='file']").val().length <= 0) { //check file selection
                message = "Lütfen dosya seçin!";
                hasError = true;
            }
            else if (allowFileCheck && $(field).find("input[type='file']").length > 0) { //check file extension
                ...
                if (!aware.checkExtension(fileName, allowedExtensions)) {
                    message = "Sadece '" + allowedExtensions + "' uzantılı dosyalar yükleyebilirsiniz!";
                    hasError = true;
                }
            }
            ...
            if (hasError) {
                if (message.length == 0) {
                    ...
                    title = title || "Bu alan için değer";
                    message = $(field).data("validation-message") || (title + " belirtmediniz!");
                }

                validated = false;
                //aware.showToastr(message, 'error');
                $(field).addClass('error');
				$(field).parent().find("span.error").remove();
				$(field).parent().append("<span class='error'><i class='fa fa-exclamation-circle'></i> "+message+"</span>");
                //current[0].showTooltip(field, message, pos);
            }
        });

        if (!validated) {
            $(area).find(".validate.error:eq(0)").focus();
        }
        return validated;
    };

    this.instantValidate = function (area, pos) {
        $(area + " input.validate").on("keyup blur paste", {}, function () {
            var field = $(this);
            var value = $(field).val().trim();
            if (!value) {
                $(field).addClass('error');
        ...
        $(area + " select.validate").on("change blur", {}, function () {
            var field = $(this);
            var value = $(field).val();
            if (!value || value == 0) {
                $(field).addClass('error');
    ...
    this.processRequest = function (url, postData, loadingMessage, onSuccess, onError) {
        this.showLoading(loadingMessage, true);
        $.post(url, postData, function (data) {
            if (data.success == 1) {
                this.showMessage(undefined, data.message);
    ...
    this.asDatePicker = function (elem) {
        try {
            $(elem).datepicker({
    ...
    this.confirm = function (message, onConfirm, title) {
        ...
        $("#confirmModal").find(".title span").html(title);
    ...
    this.QS_refresh = function (key, value, query) {
        var result = '';
        query = query != undefined ? query : (window.location.href.split('?')[1] || '');
