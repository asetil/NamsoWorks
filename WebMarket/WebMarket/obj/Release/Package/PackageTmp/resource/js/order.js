    ...
    var handlers = {
        ...
        onBeforeApproveOrder: function () {
            ...
            $.post("/Order/SavePaymentInfo", { orderID: orderID, paymentType: paymentType, subPaymentType: subPaymentType, installment: installmant }, function (response) {
                if (response.success == 1) {
                    if (response.order.PaymentType == 3) { //KREDİ KARTI İLE
                        ...
                        aware.delayedRefresh(3600, "siparis-detay-" + response.idinfo);
                    }
        ...
        processPayment: function (order) {
            var cardInfo = $("#payment_creditcard form").serializeObject();
            ...
            $.post("/Order/ProcessPayment", postData, function (response) {
                if (response.success == 1) {
                    ...
                    if (response.code == 1) { //3D form
                        $(".payment-modal .payment-panel").html(response.value);
                        $(".payment-modal .loading").removeClass("dn");
                        $(".payment-modal").removeClass("bigger");
                        $(".payment-modal .payment-panel #PaymentForm").attr("target", "paymentFrame");
                        elem.redirectedToBank = 0;

                        setTimeout(function () {
                            $(".payment-modal .payment-panel #PaymentForm").submit();
                            $(elem.paymentIframe).attr("scrolling", "yes");
                            elem.redirectedToBank = 1;
                        }, 600);

                        $(".payment-modal #paymentFrame").attr("src", "").hide();
        ...
        onPaymentFrameLoad: function () {
            if (elem.redirectedToBank == 1) {
                $(".payment-modal #paymentFrame").show();
                $(".payment-modal").addClass("bigger");
                $(".payment-modal .loading").addClass("dn");
                $(elem.paymentIframe).attr("scrolling", "yes");
                elem.redirectedToBank = 2;
            }
            else if (elem.redirectedToBank == 2) {
                $(".payment-modal .loading").addClass("dn");
                $(elem.paymentIframe).attr("scrolling", "no");
                $(".payment-modal").removeClass("bigger").addClass("big");
                elem.redirectedToBank = 0;
            }
        ...
        closePaymentModal: function (success) {
            $(".payment-modal .loading").addClass("dn");
            $(".payment-modal").removeClass("bigger").removeClass("big");
            elem.redirectedToBank = 0;

            if (success) {
                aware.showLoading("Sipariş detayına yönlendiriliyorsunuz...");
                aware.delayedRefresh(1000, "/siparis-detay-" + $("#orderUniqueID").val());
            } else {
        ...
        ready: function () {
            ...
            $(elem.paymentIframe).load(handlers.onPaymentFrameLoad);
            window.closePaymentModal = handlers.closePaymentModal;
    ...
    $.post("/Order/ProcessPayment", postData, function (response) {
        if (response.success == 1) {
            ...
            if (response.code == 1) { //3D form
                ...
                setTimeout(function () {
                    $(".payment-modal .payment-panel #PaymentForm").submit();
                    $("#paymentFrame").attr("scrolling", "yes");
                    $(".payment-modal").addClass("osman");
                    //elem.redirectedToBank = 1;
                }, 600);
