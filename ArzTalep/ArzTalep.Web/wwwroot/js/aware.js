$.awareValidation = function () {
    ...
    var nullCheck = function (field, value) {
        ...
        return { valid: valid, message: valid ? "" : getTitle(field) + " belirtmediniz!" };
    }
    ...
    var eqaulityCheck = function (container, field, value) {
        ...
        if (!valid) {
            var compareId = $(field).data("compare-id");
            if (compareId) {
                var comparedField = $(container).find("#" + compareId);
                ...
                valid = true;
                var compareField = $(container).find("[data-compare-id='" + $(field).attr("id") + "']");
                $this.validateField(compareField, container);
            }
        }
    ...
    var handleValidation = function (container) {
        ...
        $(container).find(".validation-desc").remove();

        $(container).find(".validate").each(function () {
            var field = $(this);
            var result = $this.validateField(field, container);
        ...
        if (!validated) {
            $(container).find(".validate.invalid:eq(0)").focus();
        }
        return validated;
    }

    this.validateField = function (field, container) {
        var result = undefined;
        try {
            var value = $(field).val().trim();
            result = nullCheck(field, value);
            ...
            $(field).parent().find(".validation-desc").remove();
            if (!result.valid) {
                validated = false;
                $(field).addClass('invalid');
                $(field).removeClass('valid');

                var errorSpan = "<span class='validation-desc'>" + result.message + "</span>";
                $(field).after(errorSpan);
            }
            else {
                $(field).addClass('valid');
                $(field).removeClass('invalid');
                var checkSpan = "<i class='validation-desc fa fa-check'></i>";
                $(field).after(checkSpan);
            }
        } catch (ex) {
            console.log("Aware > Validation Error", ex);
            result = { valid: false, message: "Lütfen giriş yaptığınız değerleri kontrol edin!" };
        }
        return result;
    }
    ...
    this.instantValidate = function (container) {
        $(container + " input.validate").on("keyup blur paste", {}, function () {
            var field = $(this);
            $this.validateField(field, container);
        });
...
$.AwareSelecto = function () {
    ...
    var drawFiltered = function (definition) {
        ...
        html += '<input type="hidden" id="' + idInfo + '" name="' + nameInfo + '" value="' + selectedID + '"/>';
        html += '<input type="text" class="txt searchable" value="' + selectedText + '" placeholder="Ara.."/>';
